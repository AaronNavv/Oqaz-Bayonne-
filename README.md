...
<script>
    ...
    stars.forEach(star => {
        star.addEventListener('click', () => {
            ...
            btn.style.display = 'inline-block'; // Affiche le bouton après le choix
        });
    });
    ...
    btn.addEventListener('click', () => {
        ...
        const emailContact = "oqaz.jim@gmail.com";

        if (selectedRating >= 4) {
            // Redirection Google
            window.location.href = googleLink;
        } else {
            // Redirection Email avec sujet pré-rempli
            const subject = encodeURIComponent("Retour d'expérience client");
